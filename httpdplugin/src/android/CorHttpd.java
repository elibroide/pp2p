...
public class CorHttpd extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray inputs, CallbackContext callbackContext) throws JSONException {
        ...
        if (ACTION_START_SERVER.equals(action)) {
            ...
            result = send(inputs, callbackContext);
            
        } else if (ACTION_CLOSE.equals(action)) {
            Log.d(LOGTAG, String.format("Unhandled action passed: %s", action));
    ...
    private String __startServer() {
    	String errmsg = "";
    	try {
            server = new WebServer(this, localPath, port);
            server.start(0);
		} catch (IOException e) {
   ...
   private PluginResult getURL(JSONArray inputs, CallbackContext callbackContext) {
    ...
    private PluginResult send(JSONArray inputs, CallbackContext callbackContext){
        try{
            server.send(inputs.optString(0), inputs.optString(1));
        } catch(Exception ex){
