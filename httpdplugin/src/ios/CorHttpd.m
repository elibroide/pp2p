...
- (void)send:(CDVInvokedUrlCommand *)command
{
    ...
    MyWebSocket* socket = self.sockets[[command.arguments objectAtIndex:0]];
    if(socket == nil){
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"No such socket"];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
        return;
    }
    [socket sendMessage:[command.arguments objectAtIndex:1]];
